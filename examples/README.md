# hevsearch Python examples

Runnable examples for the `hevsearch` Python package (built from `../python`).
...
Creates a virtualenv and installs `hevsearch` (from the local wheel in
...
Writes to `./hevsearch_data_demo`, then runs vector, full-text (BM25), and
...
export TIGRIS_BUCKET=hevsearch-tigris-bucket   # optional; this is the default
./run.sh clip
```

Embeds a few sample photos with OpenCLIP, stores the photo bytes as
Tigris objects and the embeddings in hevsearch (also on Tigris), then

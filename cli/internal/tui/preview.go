...
func newPreviewModel(docID, content string) previewModel {
	return previewModel{docID: docID, content: content}
}
...
func (m previewModel) view(width, height int) string {
	...
	b.WriteString(headerStyle.Render(fmt.Sprintf(" Document: %s ", m.docID)))
	b.WriteString("\n\n")
	...
	b.WriteString(helpStyle.Render(fmt.Sprintf("↑/k ↓/j • enter full view • y copy • esc back  %s", scrollPct)))
	return b.String()
}

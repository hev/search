...
func TestQueryRequestConstruction(t *testing.T) {
	...
	c := newTestClient(srv.URL, config.Endpoint{APIKey: "read-key"})
	text := "hello"
	...
	assert.Equal(t, "Bearer read-key", gotAuth)
...
func TestAdminEndpointUsesAdminKey(t *testing.T) {
	...
	c := newTestClient(srv.URL, config.Endpoint{APIKey: "read", AdminAPIKey: "admin"})
	res, _, err := c.DeleteNamespace(t.Context(), "docs")
	require.NoError(t, err)
	assert.Equal(t, http.MethodDelete, gotMethod)
	assert.Equal(t, "/ns/docs", gotPath)
	assert.Equal(t, "Bearer admin", gotAuth)
...
func TestNoAuthHeaderWhenUnconfigured(t *testing.T) {
	...
	require.NoError(t, err)
	assert.False(t, hadAuth, "no Authorization header should be sent when no key is configured")

# Index cache bench: warm-index-cache

- **Date**: 2026-05-25
- **Mode**: `warm-index-cache`
...
| p50 | p95 | p99 | max |
| ---: | ---: | ---: | ---: |
| 26.14 ms | 37.21 ms | 45.76 ms | 54.63 ms |
...
Hit-rate signal: `insert` deltas approach zero as Lance finds its index state already cached. A non-trivial `insert` count after a warmup pass would mean Lance is still opening fresh state under load, which is worth a second look.

## Firn-level S3 requests (delta)

| counter | delta |
| --- | ---: |
| `firnflow_s3_requests_total` for this namespace | 1000 |

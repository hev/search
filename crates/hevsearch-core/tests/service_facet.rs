...
use hevsearch_core::metrics::{test_metrics, CoreMetrics};
use hevsearch_core::{
...
async fn facet_repeats_hit_cache_and_filters_do_not_collide() {
    ...
    assert_eq!(metric_value(&body, "hevsearch_cache_misses_total", &ns), 1);
    assert_eq!(metric_value(&body, "hevsearch_cache_hits_total", &ns), 1);
    ...
    let body = metrics.encode().unwrap();
    assert_eq!(metric_value(&body, "hevsearch_cache_misses_total", &ns), 2);
    ...
    let body = metrics.encode().unwrap();
    assert_eq!(metric_value(&body, "hevsearch_cache_misses_total", &ns), 3);
}

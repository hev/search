...
//! ./scripts/cargo test -p hevsearch-core --test manager_import \
//!     -- --ignored --nocapture
//! ```
...
use hevsearch_core::{HevSearchError, NamespaceId, NamespaceManager, StorageRoot};
...
async fn import_rejects_null_id() {
    ...
    assert!(
        matches!(err, HevSearchError::InvalidRequest(_)),

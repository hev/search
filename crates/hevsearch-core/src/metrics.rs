...
//! exposes:
//!
//! * `hevsearch_cache_hits_total{namespace}`
//! * `hevsearch_cache_misses_total{namespace}`
//! * `hevsearch_semantic_cache_hits_total{namespace}`
//! * `hevsearch_semantic_cache_misses_total{namespace}`
//! * `hevsearch_semantic_cache_rejections_total{namespace, reason}`
//! * `hevsearch_query_duration_seconds{namespace, query_type}`
//! * `hevsearch_write_duration_seconds{namespace}`
...
//! `hevsearch-api::state::build_state`), wrapped in `Arc`, and
...
use crate::{HevSearchError, NamespaceId};
...
impl CoreMetrics {
    ...
    pub fn new() -> Result<Self, HevSearchError> {
        let registry = Registry::new();

        let cache_hits = IntCounterVec::new(
            Opts::new(
                "hevsearch_cache_hits_total",
        ...
        let semantic_cache_hits = IntCounterVec::new(
            Opts::new(
                "hevsearch_semantic_cache_hits_total",
        ...
        let semantic_cache_misses = IntCounterVec::new(
            Opts::new(
                "hevsearch_semantic_cache_misses_total",
        ...
        let semantic_cache_rejections = IntCounterVec::new(
            Opts::new(
                "hevsearch_semantic_cache_rejections_total",
        ...
        let query_duration = HistogramVec::new(
            HistogramOpts::new(
                "hevsearch_query_duration_seconds",
        ...
        let write_duration = HistogramVec::new(
            HistogramOpts::new(
                "hevsearch_write_duration_seconds",
        ...
        let s3_requests = IntCounterVec::new(
            Opts::new(
                ...
                "hevsearch-initiated operations that bypassed the cache \
        ...
        let index_build_duration = HistogramVec::new(
            HistogramOpts::new(
                "hevsearch_index_build_duration_seconds",
        ...
        let compaction_duration = HistogramVec::new(
            HistogramOpts::new(
                "hevsearch_compaction_duration_seconds",
        ...
        let cached_handles = IntGauge::new(
            ...
            "Number of namespaces with a warm `lancedb::Connection` + \
             ...
             against `hevsearch_active_namespaces` — the delta is the \
    ...
    pub fn encode(&self) -> Result<String, HevSearchError> {
        let mut buffer = Vec::new();
        let encoder = TextEncoder::new();
        encoder
            .encode(&self.registry.gather(), &mut buffer)
            .map_err(metrics_err)?;
        String::from_utf8(buffer).map_err(|e| HevSearchError::Metrics(e.to_string()))
    }
    ...
    /// Current value of `hevsearch_semantic_cache_hits_total{namespace=…}`.
    ...
    /// Current value of `hevsearch_semantic_cache_misses_total{namespace=…}`.
    /// Test-only accessor; production reads `/metrics`.
    ...
    /// Record a hevsearch-initiated S3-bound operation. See the
    /// help text on `hevsearch_s3_requests_total` for why this is
    /// an approximation.
    ...
    /// Current value of the `hevsearch_cached_handles` gauge. Used
...
fn metrics_err(e: impl std::fmt::Display) -> HevSearchError {
    HevSearchError::Metrics(e.to_string())
}

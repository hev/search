...
impl NamespaceCache {
    ...
    pub async fn new(
        ...
        nvme_bytes: usize,
        metrics: Arc<CoreMetrics>,
    ) -> Result<Self, HevSearchError> {
        let device = FsDeviceBuilder::new(nvme_path)
            .with_capacity(nvme_bytes)
            .build()
            .map_err(|e| HevSearchError::Cache(format!("device build: {e}")))?;
        ...
        let cache = HybridCacheBuilder::new()
            ...
            .build()
            .await
            .map_err(|e| HevSearchError::Cache(format!("hybrid build: {e}")))?;
    ...
    pub async fn get_or_populate<F, Fut>(
        ...
        query: QueryHash,
        populate: F,
    ) -> Result<Vec<u8>, HevSearchError>
    where
        F: FnOnce() -> Fut,
        Fut: Future<Output = Result<Vec<u8>, HevSearchError>>,
    {
    ...
    /// this returns.
    pub async fn close(&self) -> Result<(), HevSearchError> {
        self.cache
            .close()
            .await
            .map_err(|e| HevSearchError::Cache(format!("cache close: {e}")))
    }
}

...
//! HEVSEARCH_BENCH_DIM=1536 \
//! HEVSEARCH_BENCH_ROWS=100000 \
//!   ./scripts/cargo run --release -p hevsearch-bench
//! ```
//!
//! Env vars: set `HEVSEARCH_STORAGE_URI` (preferred) or the legacy
//! `HEVSEARCH_S3_BUCKET` — at least one must be set, and if both are
//! set the URI wins. `HEVSEARCH_STORAGE_URI` accepts both `s3://...`
//! and `gs://...`; the legacy var is S3-only. Everything else is
//! optional.
//!
//! | var                             | default                                    |
//! | ------------------------------- | ------------------------------------------ |
//! | `HEVSEARCH_STORAGE_URI`          | *(required, or set `HEVSEARCH_S3_BUCKET`)*  |
//! | `HEVSEARCH_S3_BUCKET`            | *(legacy fallback for the URI)*            |
...
//! | `HEVSEARCH_BENCH_ROWS`           | `100`                                      |
//! | `HEVSEARCH_BENCH_QUERIES`        | `50`                                       |
//! | `HEVSEARCH_BENCH_OUT`            | `bench/results/cold_vs_warm_realistic.md`  |
//! | `HEVSEARCH_BENCH_CACHE_RAM_MB`   | `16`                                       |
//! | `HEVSEARCH_BENCH_CACHE_NVME_MB`  | `256`                                      |
//! | `HEVSEARCH_BENCH_INDEX_NPROBES`  | `20`                                       |
...
impl BenchConfig {
    fn from_env() -> anyhow::Result<Self> {
        // Prefer HEVSEARCH_STORAGE_URI (so a fixed prefix in the
        // operator config carries through to the bench), fall back
        // to the legacy HEVSEARCH_S3_BUCKET. No strict disagreement
        // check here — bench is a dev tool, not a deployment
        // surface. Empty strings count as unset so accidental
        // `HEVSEARCH_STORAGE_URI=` exports don't shadow the bucket
        // fallback.
        ...
        let dim: usize = env_or("HEVSEARCH_BENCH_DIM", "32")
            .parse()
            .context("HEVSEARCH_BENCH_DIM")?;
        let rows: usize = env_or("HEVSEARCH_BENCH_ROWS", "100")
            .parse()
            .context("HEVSEARCH_BENCH_ROWS")?;
        let queries: usize = env_or("HEVSEARCH_BENCH_QUERIES", "50")
            .parse()
        ...
        let cache_ram_mb: usize = env_or("HEVSEARCH_BENCH_CACHE_RAM_MB", "16")
            .parse()
            .context("HEVSEARCH_BENCH_CACHE_RAM_MB")?;
        let cache_nvme_mb: usize = env_or("HEVSEARCH_BENCH_CACHE_NVME_MB", "256")
            .parse()
            .context("HEVSEARCH_BENCH_CACHE_NVME_MB")?;
        let nprobes: usize = env_or("HEVSEARCH_BENCH_INDEX_NPROBES", "20")
            .parse()
            .context("HEVSEARCH_BENCH_INDEX_NPROBES")?;
...
- `s3_requests_total` counts hevsearch-initiated operations at the service \

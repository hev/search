...
impl BenchConfig {
    fn from_env() -> anyhow::Result<Self> {
        ...
        let dim = parse_env("HEVSEARCH_BENCH_DIM", 512usize)?;
        let rows = parse_env("HEVSEARCH_BENCH_ROWS", 10_000usize)?;
        let reps = parse_env("HEVSEARCH_SEM_REPS", 200usize)?;
        let k = parse_env("HEVSEARCH_SEM_K", 10usize)?;
        let nprobes = parse_env("HEVSEARCH_BENCH_INDEX_NPROBES", 20usize)?;
        let threshold = parse_env("HEVSEARCH_SEM_THRESHOLD", 0.995f32)?;
        let cosine_sweep = parse_f32_list(
            "HEVSEARCH_SEM_COSINE_SWEEP",
            "0.999,0.997,0.995,0.99,0.97,0.95,0.90,0.85",
        )?;
        let fill_sizes = parse_usize_list("HEVSEARCH_SEM_FILL", "1,16,128,512,1024")?;
        let out_path = PathBuf::from(env_or(
            "HEVSEARCH_SEM_OUT",
            "bench/results/semantic_cache_profile.md",
        ));
        let cache_ram_mb = parse_env("HEVSEARCH_BENCH_CACHE_RAM_MB", 16usize)?;
        let cache_nvme_mb = parse_env("HEVSEARCH_BENCH_CACHE_NVME_MB", 256usize)?;

        if dim == 0 {
            anyhow::bail!("HEVSEARCH_BENCH_DIM must be > 0");
        }
        if rows == 0 {
            anyhow::bail!("HEVSEARCH_BENCH_ROWS must be > 0");
        }
        if reps == 0 {
            anyhow::bail!("HEVSEARCH_SEM_REPS must be > 0");
        }
        if k == 0 {
            anyhow::bail!("HEVSEARCH_SEM_K must be > 0");
        }
        if !threshold.is_finite() || threshold <= 0.0 || threshold > 1.0 {
            anyhow::bail!("HEVSEARCH_SEM_THRESHOLD must be in (0.0, 1.0]");
        }
        for c in &cosine_sweep {
            if !c.is_finite() || *c < 0.0 || *c > 1.0 {
                anyhow::bail!("HEVSEARCH_SEM_COSINE_SWEEP values must be in [0.0, 1.0]");
            }
        }
        if fill_sizes.is_empty() {
            anyhow::bail!("HEVSEARCH_SEM_FILL must contain at least one entry");
        }
...
fn snapshot(metrics: &CoreMetrics, ns: &NamespaceId) -> MetricSnapshot {
    ...
    MetricSnapshot {
        cache_hits: metric_value(&body, "hevsearch_cache_hits_total", &[&namespace_label]),
        cache_misses: metric_value(&body, "hevsearch_cache_misses_total", &[&namespace_label]),
...
- **Start UTC**: {start_utc}\n\
- **Stop UTC**: {stop_utc}\n\
- **Harness**: `./scripts/cargo run --release -p hevsearch-bench --bin semantic_cache_profile -j 1`\n\

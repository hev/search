...
async fn missing_header_returns_401() {
    ...
    assert_eq!(www, "Bearer realm=\"hevsearch\"");
}
...
async fn duplicate_keys_fail_startup_before_cache_setup() {
    ...
    let unique = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let blocker = not_a_directory.join(format!("hevsearch-not-a-dir-{unique}"));
    ...
    let msg = format!("{err:#}");
    assert!(
        msg.contains("HEVSEARCH_API_KEY") && msg.contains("HEVSEARCH_ADMIN_API_KEY"),

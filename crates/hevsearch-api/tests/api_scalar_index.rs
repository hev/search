...
//!   `hevsearch_index_build_duration_seconds_count{kind="scalar"}`
...
//! ./scripts/cargo test -p hevsearch-api --test api_scalar_index \
//!     -- --ignored --nocapture
//! ```
...
/// `hevsearch_index_build_duration_seconds_count` for this
...
async fn scalar_index_build_returns_202_and_list_still_works() {
    ...
    let sum =
        metric_value(&body, "hevsearch_index_build_duration_seconds_sum", &label).unwrap_or(0.0);
    assert!(
        sum > 0.0,

...
//! `hevsearch_index_build_duration_seconds` histogram records the
//! build time.
//!
...
//! ./scripts/cargo test -p hevsearch-api --test api_index -- --ignored --nocapture
//! ```
...
async fn index_build_returns_202_and_records_metric() {
    ...
    let sum = metric_value(
        &body,
        "hevsearch_index_build_duration_seconds_sum",
        &ns_label,
    )
    .unwrap_or(0.0);
    assert!(

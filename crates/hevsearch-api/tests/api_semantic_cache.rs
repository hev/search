...
//!   the `hevsearch_semantic_cache_hits_total` counter ticks.
...
async fn build_app() -> (axum::Router, hevsearch_api::AppState, tempfile::TempDir) {
...
async fn post_json_debug(
    ...
    let request = Request::builder()
        ...
        .header("x-hevsearch-debug-cache-source", "true")
...
async fn api_semantic_cache_near_duplicate_hits_and_metric_ticks() {
    ...
    let needle = format!("hevsearch_semantic_cache_hits_total{{namespace=\"{ns}\"}} 1");
    assert!(

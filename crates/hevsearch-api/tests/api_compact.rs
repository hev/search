...
//! `hevsearch_compaction_duration_seconds` histogram recorded the
...
//! ./scripts/cargo test -p hevsearch-api --test api_compact -- --ignored --nocapture
//! ```
...
async fn wait_for_compaction(
    ...
    loop {
        ...
        let count = metric_value(&body, "hevsearch_compaction_duration_seconds_count", &label)
            .map(|v| v as u64)
            .unwrap_or(0);

//! Request handlers for the hevsearch REST API.
//!
//! * `GET    /health`
...
const CACHE_SOURCE_REQUEST_HEADER: &str = "x-hevsearch-debug-cache-source";
const CACHE_SOURCE_RESPONSE_HEADER: &str = "x-hevsearch-cache-source";
...
/// completion, or watch the `hevsearch_index_build_duration_seconds`
/// histogram.
pub async fn create_index(
    ...
    if req.kind != "ivf_pq" {
        return Err(ApiError::Core(
            hevsearch_core::HevSearchError::InvalidRequest(format!(
    ...
    hevsearch_core::validate_ivf_pq_options(req.num_bits, req.num_sub_vectors)
...
/// `hevsearch_index_build_duration_seconds{kind="scalar"}`.
///
...
pub async fn create_scalar_index(
    ...
    if hevsearch_core::validate_scalar_index_column(&column).is_err() {
        if matches!(column.as_str(), "vector" | "vectors" | "text") || column.starts_with('_') {
            hevsearch_core::validate_scalar_index_column(&column).map_err(ApiError::Core)?;
        }
        state
            .manager
...
/// `HEVSEARCH_MAX_BODY_BYTES` (this route disables that limit). The whole
/// stream is appended in a single Lance commit, insert-only — see
/// [`hevsearch_core::NamespaceManager::import_arrow`] for the schema
...
/// temp file, enforcing `HEVSEARCH_IMPORT_MAX_BYTES` (413 if exceeded) →
...
pub async fn import(
    ...
    let tmp = tempfile::Builder::new()
        .prefix("hevsearch-import-")
        .tempfile_in(&state.import_tmp_dir)
        .map_err(|e| HevSearchError::Backend(format!("import: create temp file: {e}")))?;
    let mut writer = tokio::fs::File::from_std(
        tmp.reopen()
            .map_err(|e| HevSearchError::Backend(format!("import: open spool file: {e}")))?,
    );
    ...
    while let Some(chunk) = stream.next().await {
        let chunk = chunk
            .map_err(|e| HevSearchError::InvalidRequest(format!("import: body read error: {e}")))?;
        written += chunk.len() as u64;
        if cap != 0 && written > cap {
            return Ok(status_error(
                StatusCode::PAYLOAD_TOO_LARGE,
                format!("import body exceeds HEVSEARCH_IMPORT_MAX_BYTES ({cap} bytes)"),
            ));
        }
        writer
            .write_all(&chunk)
            .await
            .map_err(|e| HevSearchError::Backend(format!("import: spool write: {e}")))?;
    }
    writer
        .flush()
        .await
        .map_err(|e| HevSearchError::Backend(format!("import: spool flush: {e}")))?;
    ...
    let schema = {
        let file = tmp
            .reopen()
            .map_err(|e| HevSearchError::Backend(format!("import: reopen temp: {e}")))?;
        let reader = StreamReader::try_new(BufReader::new(file), None).map_err(|e| {
            HevSearchError::InvalidRequest(format!("import: not a valid Arrow IPC stream: {e}"))
        })?;
        reader.schema()
    };
...
/// `hevsearch_compaction_duration_seconds` histogram.
...
pub async fn list(
    ...
    if let Some(col) = params.order_by.as_deref() {
        if col != LIST_ORDER_BY {
            return Err(ApiError::Core(HevSearchError::InvalidRequest(format!(
    ...
    if limit == 0 {
        return Err(ApiError::Core(HevSearchError::InvalidRequest(
...
fn operation_error_message(err: &HevSearchError) -> String {
    match err {
        HevSearchError::InvalidNamespace(msg) => format!("invalid namespace: {msg}"),
        HevSearchError::InvalidRequest(msg) => format!("invalid request: {msg}"),
        HevSearchError::Unsupported(msg) => format!("unsupported: {msg}"),
        HevSearchError::Backend(_)
        | HevSearchError::Cache(_)
        | HevSearchError::Io(_)
        | HevSearchError::Metrics(_) => "operation failed".to_string(),
    }
}
...
mod tests {
    ...
    fn operation_error_message_surfaces_caller_errors() {
        let msg = operation_error_message(&HevSearchError::Unsupported(
        ...
        let msg = operation_error_message(&HevSearchError::InvalidRequest(
    ...
    fn operation_error_message_scrubs_backend_internals() {
        let leaky = HevSearchError::Backend(

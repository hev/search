"""End-to-end tests for the hevsearch package against local embedded mode.
...
def db(tmp_path, monkeypatch):
    ...
    client = hevsearch.connect(str(tmp_path / "data"))
    yield client
    client.close()
...
def test_hybrid_guards(db):
    ...
    with pytest.raises(hevsearch.ValidationError):
...
def test_bad_tenant_rejected(db):
    with pytest.raises(hevsearch.TenantError):
...
def test_delete(db):
    ...
    with pytest.raises(hevsearch.UnsupportedError):
        db.delete(ids=[1])
...
def test_close_is_idempotent_and_blocks_use(tmp_path, monkeypatch):
    ...
    with pytest.raises(hevsearch.HevSearchError):
    ...
    with pytest.raises(hevsearch.HevSearchError) as exc:
        client.delete(ids=[1])
    assert not isinstance(exc.value, hevsearch.UnsupportedError)
...
def test_concurrent_search_and_close(tmp_path, monkeypatch):
    ...
    def worker():
        for _ in range(20):
            ...
            except hevsearch.HevSearchError:
